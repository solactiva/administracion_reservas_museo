<template>
	...
	>
		...
		<Column bodyStyle="text-align: center; overflow: visible" style="width: 5%">
			<template #body="{ data }">
				<Button
					type="button"
					icon="pi pi-eye"
...
const ManageReservaFooter = defineAsyncComponent(() =>
	import('@/components/Reservas/ManageReservaFooter.vue')
)

const dialog = useDialog()
const route = useRoute()
const { interactividad, reservasPendientes, loadReservas } = useReservas()
...
const verReserva = (identificador) => {
	const tipoReserva = reservasPendientes.value.find(
		(el) => el.identificador === identificador
	).tipoReserva
	const modificar = tipoReserva === 'call' ? true : false
	openModalManageReserva(identificador, modificar)
}
...
const openModalManageReserva = (identificador, modificar) => {
	...
	const dialogRef = dialog.open(ManageReservaForm, {
		...
		templates: {
			footer: markRaw(ManageReservaFooter),
		},
		data: {
			identificador,
			modificar,
